...
<tr><td>Abgabe</td><td><div class="paare">{% if abg_z != "" or abg_p != "" %}<span class="f">ZS:</span><span>{% if abg_z != "" %}{{ abg_z }}{% else %}{{ abgabe_offen }}{% endif %}</span><span class="f">PS:</span><span>{% if abg_p != "" %}{{ abg_p }}{% else %}{{ abgabe_offen }}{% endif %}</span>{% else %}<span class="f">Termin:</span><span>{{ abgabe_offen }}</span>{% endif %}<span class="voll ohne-einzug">digital als PDF und docx-Datei</span><span class="voll"><span class="zeile"><span class="fi">an:</span><a href="mailto:sekretariat.europarecht@uni-leipzig.de">sekretariat.europarecht@<wbr>uni-leipzig.de</a></span><span class="zeile"><span class="fi">cc:</span>{% include email.html nur="haupt" %}</span></span><span class="voll ohne-einzug">gedruckt im Sekretariat der Professur<br>(Burgstraße&nbsp;21, Raum&nbsp;1.26)</span></div></td></tr>
