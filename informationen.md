...
<tr><td>E&#8209;Mail</td><td>{% include email.html form="zeilen" %}</td></tr>
...
<tr class="abschnitt"><td><span class="zwei">{{ site.seminar.lehrform }}{% if sws != "" %}<br><span class="sws">({{ sws }})</span>{% endif %}</span></td><td><div class="paare"><span class="f">Termin:</span><span>voraussichtlich {{ site.termine.block }}{% if site.termine.block_hinweis != "" %}; {{ site.termine.block_hinweis }}{% endif %}</span><span class="f">Raum:</span><span>{% if raum != "" %}{{ raum }}{% else %}wird hier ergänzt{% endif %}</span><span class="f">Empfohlen:</span><span>{{ site.seminar.fachsemester | replace: "; ", "<br>" }}</span>{% if swp != "" %}<span class="f">Schwerpunkt:</span><span>{{ swp | replace: "; ", "<br>" }}</span>{% endif %}</div></td></tr>
...
<tr><td>Bearbeitungszeit</td><td><div class="paare"><span class="f voll">Beginn:</span><span class="voll">{% if beginn != "" %}{{ beginn }}{% else %}{{ beginn_offen | replace: "; ", "<br>" }}{% endif %}</span><span class="f">Zulassungsseminar (ZS):</span><span>9&nbsp;Wochen</span><span class="f">Prüfungsseminar (PS):</span><span>8&nbsp;Wochen</span></div></td></tr>
